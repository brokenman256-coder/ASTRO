...
export default function AdminAstrologersPage() {
  ...
  async function runBulkSeed() {
    ...
    if (count < 1) return;
    if (!confirm(`Create ${count} Oracle profiles now? This uses placeholder avatars (no AI image cost) and can't be undone in bulk - you'd have to retire them one by one.`)) {
      return;
    }
    setBusy(true);
    try {
...
function toPayload(f: PersonaFormState) {
  ...
  return (
    <AdminGuard>
      <AdminNav />
      <h1 className="text-2xl font-semibold mb-2">Oracle roster</h1>
      <p className="text-slate-500 text-sm mb-6">
        Use the bot to automatically add fresh Oracle profiles or retire stale/low-rated ones -
      ...
      <div className="card p-6 mb-8 space-y-3">
        <h2 className="font-medium">Bulk-generate Oracles</h2>
      ...
      {scheduler && (
        <div className="card p-6 mb-8 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                When on, the bot adds a new Oracle every {scheduler.intervalMinutes} minute
          ...
          <div className="border-t border-orange-100 pt-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                ...
                for {scheduler.refreshBatchSize} random bot-generated Oracles (never the hand-curated named
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-xs text-slate-500 block mb-1">Oracles refreshed per run</label>
              <input
                className="input"
                type="number"
                min={1}
      ...
      <form onSubmit={handleManualAdd} className="card p-6 space-y-3 mb-8">
        <h2 className="font-medium">Add a new AI Oracle</h2>
        <p className="text-xs text-slate-500">
          Create a fully independent AI persona - no code changes needed. The system instructions
          define how this Oracle talks and what it focuses on.
        </p>
        <PersonaFields value={form} onChange={setForm} />
        <button className="btn-primary" disabled={busy}>Create Oracle</button>
      </form>
...
function PersonaFields({
  ...
  return (
    <>
      ...
      <textarea
        className="input"
        placeholder="AI system instructions - how this Oracle should talk and what to focus on"
        rows={4}
